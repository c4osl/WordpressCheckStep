	...
	class BBoss_Updates_Helper {
		...
		function plugin_api_call( $def, $action, $args ) {
			...
			if ( ! empty( $raw_response ) && is_wp_error( $raw_response ) ) {
				...
				if ( false === $res ) {
					$error_message = ! empty( $raw_response ) ? $raw_response['body'] : __( 'An unknown error occurred', 'buddyboss-pro' );
					$res           = new WP_Error( 'plugins_api_failed', __( 'An unknown error occurred', 'buddyboss-pro' ), $error_message );
				}
			}
